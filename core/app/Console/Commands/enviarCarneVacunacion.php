...
class enviarCarneVacunacion extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'emp:CarneVacunacion';
    ...
    public function handle()
    {
        $fecha =new Carbon('first day of next month');
            ...
            $empleados =  Terceros::select(
                ...
                ->where(
                    'cz9_fv_carne_vacunacion', '>=',  $fecha
                )
                ->where('cz9_fv_carne_vacunacion', '<=', $fecha2 )
                ->get();
                 
           if($empleados != '[]'){
            Mail::to('adavidparra0412@gmail.com')->send(new CarneVacunacion($empleados, $fecha, $fecha2));
           }
    }
}
